...
- **M6 — Past Orders + search/filter, activity history views.**
  ✅ **Done.** Search/filter at `/orders/past` (property/retailer/cleaner/delivery-status/order-number/item/date), plus `orders_with_status.had_issue` for historical issue tracking. "Activity history views" specifically didn't get a dedicated screen — the `activity_log` gap noted in Section 11 is still open; what M6 actually needed from history (who confirmed what, when, delivery provenance) was added to the existing `/orders/[id]` page instead.
- **M7 — Carrier tracking API integration (supplemental signal) + the simple daily-digest reminder.**
  **Partially done, by explicit decision.** The carrier-tracking API half was deliberately skipped, not deferred by oversight — evaluated current AfterShip/EasyPost-style pricing ($10–70+/month depending on volume/tier) against the fact that the email pipeline already updates status when retailers send it and cleaner confirmation is the real source of truth regardless; no evidence yet that packages get stuck on stale status often enough to justify the ongoing cost. Revisit if real usage shows otherwise. The daily-digest reminder is ✅ done: a package `delivered` for more than 24h without cleaner confirmation surfaces as an "Overdue" badge on `/confirmations` and in a new "Overdue confirmations" section on `/properties` for admins — in-app only, no email/push/SMS, no schema change (built entirely on the existing `packages.status`/`delivered_at` columns), one hardcoded threshold rather than a configurable rules engine, per Section 14's own "resist the urge to build a rules engine here" guidance.
- **M8 — Hardening pass:** RLS audit, PWA install/manifest polish, mobile performance pass on a real low-end phone, error monitoring wired in, data-retention job for old raw emails/screenshots.
  ✅ **Done.** Hosted Supabase project linked/migrated, live RLS audit script verified against the real hosted project, deployed to Vercel, Sentry error monitoring wired in, PWA manifest/icons confirmed working via real "Add to Home Screen" test, admin-created cleaner logins (no email dependency), dropdown nav and checkbox property filters from real user feedback, and a real mobile-performance report (2-3s loads, no tap feedback) root-caused and fixed: deduped `getCurrentProfile()` via React `cache()`, added `SubmitButton` loading states everywhere, disabled Sentry's `tracesSampleRate`, and pinned the Vercel function region to `pdx1` near Supabase's `us-west-2` project. The performance fix is now confirmed live — `/properties` and `/confirmations` feel fine on the production site. The `activity_log` gap from Section 11 was consciously left open rather than resolved here (see `CLAUDE.md` known gaps) — it needs an explicit decision (build it, or amend Section 13's assumption), not silent deferral.
